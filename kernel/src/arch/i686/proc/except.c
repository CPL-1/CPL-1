...
static const char *m_exceptionNames[0x20] = {
...
void i686_ExceptionMonitor_ExceptionHandler(void *ctx, char *frame) {
    ...
    KernelLog_WarnMsg("CPU Exception monitor", "Unhandled interrupt: \"%s\" (%u). EIP: %p, ESP: %p, EFLAGS: %p\n", *(const char **)ctx, (((uint32_t)ctx) - ((uint32_t)m_exceptionNames)) / 4, state->eip, state->esp, state->eflags);
}

void i686_ExceptionMonitor_Initialize() {
    for (size_t i = 0; i < 0x20; ++i) {
        HAL_ISR_Handler handler = i686_ISR_MakeNewISRHandler(i686_ExceptionMonitor_ExceptionHandler, (void *)(m_exceptionNames + i));
